...
export default function Admin() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      console.error("[Admin Logs] Client error:", err);
      const errorMsg = err.response?.data?.error || "Failed to fetch logs";
      toast.error(`Error loading logs: ${errorMsg}`);
    } finally {
      setLoading(false);
    }
  }, [userId]);
  ...
  const handleResolve = async (id, outcome) => {
    if (!confirm(`Resolve market as ${outcome}? This cannot be undone.`))
      return;
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              ...
              {logs.length === 0 && (
                <p className="text-text-secondary mt-4">No logs found.</p>
              )}
            </div>
          )}
        </div>
